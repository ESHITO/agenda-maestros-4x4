	...
	async function claim(e: SubmitEvent) {
		...
		try {
			...
			if (res.ok) {
				...
				error = data.error || 'No se pudo completar la configuración. Inténtalo de nuevo.';
			}
		} finally {
			submitting = false;
		}
...
<svelte:head><title>Aceptar invitación — Calnode</title></svelte:head>
...
<div class="flex min-h-screen items-center justify-center bg-muted/30 p-6">
	<div class="w-full max-w-sm">
		<div class="mb-8 text-center">
			...
			<h1 class="text-xl font-semibold tracking-tight">Has sido invitado</h1>
		</div>

		{#if valid === null}
			<p class="text-center text-sm text-muted-foreground">Verificando tu invitación…</p>
		{:else if valid === false}
			<div class="rounded-md bg-destructive/10 px-4 py-3 text-sm text-destructive text-center">
				Este enlace de invitación ha caducado o ya fue usado. Pide a tu administrador que envíe uno nuevo.
			</div>
		{:else}
			<div class="mb-5 rounded-md bg-muted px-3 py-2 text-sm text-muted-foreground">
				Esta invitación es para <span class="font-medium text-foreground">{inviteEmail}</span>.
				No puede ser usada por nadie más.
			</div>
