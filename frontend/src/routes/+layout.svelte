<script lang="ts">
	...
	// Support is not an admin, but the member directory is one of the four things it
	// is meant to do (ListUsers in users.go now answers it), and this nav entry is the
	// only way into that page. It is the ONE adminOnly item support may see — Equipos,
	// Grabaciones and everything under Configuración stay closed, and the server-side
	// checks on those routes are what actually enforces it.
	const supportExtraHref = $derived(`${base}/members`);
	const visibleNavItems = $derived(
		navItems.filter(
			(item) =>
				(!item.adminOnly ||
					$currentUser?.is_admin ||
					($currentUser?.is_support && item.href === supportExtraHref)) &&
