...
| `REMINDER_MORNING_HOUR` | no | `08:00` | Fork: `HH:MM` (24 h) for the `booking.reminder_morning` webhook — that clock time on the meeting's day, in the **attendee's** zone. A malformed value and **the app refuses to start**. Reminder webhooks run on the in-process worker, so the instance must stay up (no sleep-when-idle): a reminder whose next one is already due when the worker gets to it is skipped, not sent late. |
