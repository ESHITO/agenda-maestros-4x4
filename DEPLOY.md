...
| `REMINDER_MORNING_HOUR` | no | `08:00` | Fork: `HH:MM` (24 h) for the `booking.reminder_morning` webhook — that clock time on the meeting's day, in the **attendee's** zone. A malformed value and **the app refuses to start**. Reminder webhooks run on the in-process worker, so the instance must stay up (no sleep-when-idle): a reminder whose next one is already due when the worker gets to it is skipped, not sent late. The owner can override it from the Webhooks page (saved in the database, which wins over this variable). |
| `REMINDER_MORNING_TIMEZONE` | no | *(empty)* | Fork: IANA zone (e.g. `America/Lima`) that pins the morning reminder for everyone: the meeting's day **in that zone**, at `REMINDER_MORNING_HOUR` in that zone. Empty = each attendee's own zone. An unknown zone and **the app refuses to start**. The owner can override it from the Webhooks page ("Recordatorio de la mañana"). |
